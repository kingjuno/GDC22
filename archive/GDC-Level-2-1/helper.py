...
def delete_task(index):
    ...
    if str(index) not in data.keys():
        return f"Error: item with priority {index} does not exist. Nothing deleted."
    else:
        del data[str(index)]
        add_task(data)
        return f"Deleted item with priority {index}"

def mark_task_done(index):
    data = read_task_as_dict()
    if str(index) not in data:
        return f"Error: no incomplete item with priority {index} exists."
